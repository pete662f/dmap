...
- builds `infra/data/tiles/world-lowres.mbtiles` from Natural Earth data through Tippecanoe
...
- applies a deterministic world-lowres vector-source patch for the Android presentation
...
- `http://localhost:8080/data/world-lowres.json`
...
It also validates that the served style still contains the `world-lowres` vector source, that fallback label layers render above water, and that representative `world-lowres` vector tile endpoints are reachable.
...
The Android build also reads `infra/tileserver/style-assets.version` and appends that version token to the style URL so regenerated style assets invalidate stale cached style responses.
...
- Outside Denmark, the app falls back to a lightweight self-hosted `world-lowres` vector basemap with land, water, borders, and low-zoom place labels served as normal TileServer vector tiles.
...
For style maintenance, the repo intentionally keeps the upstream OSM Liberty snapshot plus deterministic patch steps in `infra/scripts/patch-mobile-style.py` and `infra/scripts/patch-world-lowres-style.py`. The global fallback now comes from `infra/data/tiles/world-lowres.mbtiles`, served by TileServer as a normal vector source and referenced by the generated style as `mbtiles://{world-lowres}`. Future style work should continue through those patch layers instead of manually editing the generated `style.json`.
